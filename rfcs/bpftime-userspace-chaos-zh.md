...
**原语 1: bpf_override_return - 权威的返回值覆盖**
- 立即中断被钩取函数的执行，原始函数体不会执行
- 强制函数返回 eBPF 程序指定的新值
- 与内核 eBPF 不同，bpftime 对任何用户态函数都支持此功能
- 用途：模拟 malloc 返回 NULL（内存分配失败）、read 返回 -1（I/O 错误）等

**原语 2: bpf_probe_write_user - 运行时语义篡改**
- 允许 eBPF 程序写入目标进程的用户态内存
- 可在函数执行前修改输入参数，执行后修改输出缓冲区
- 甚至可以修改声明为 `const` 的参数（const 只是编译时约束）
- 用途：路径重定向（将 config.json 改为 /dev/null）、数据损坏注入等

**原语 3: eBPF Maps - 有状态的复杂场景**
- 通过共享内存实现的键值存储
- eBPF 程序可与外部控制平面（Chaos Daemon）通信
- 支持动态策略更新（运行时改变故障概率、目标 PID 等）
- 用途：阈值触发（第 100 次调用后才失败）、策略驱动故障（按 PID 和概率注入）

### 注入架构：基于 DaemonSet 的设计

**为什么使用 DaemonSet？**
- 节点本地访问：每个 Chaos Daemon 可直接访问本节点容器
- 命名空间隔离：使用 `nsenter` 进入目标容器命名空间
- 资源高效：每个节点一个 daemon，而非每个 pod 一个
- 与现有架构一致：符合 Chaos Mesh 当前设计模式

**eBPF 程序管理流程：**
1. 用户创建 UserspaceChaos CR
2. Controller 验证并选择目标 pod
3. Controller 根据规格生成 eBPF 程序
4. 发送到对应节点的 Daemon（通过 gRPC）
5. Daemon 编译 eBPF 程序（clang/LLVM）
6. 注入 bpftime 到目标容器
7. 加载 eBPF 程序并附加钩子
8. 故障注入激活
9. 通过 eBPF Maps 监控和动态控制
10. 清理时卸载程序和 bpftime

**eBPF 程序选择机制：**
- 基于模板的方法：预定义故障类型模板（malloc_failure, io_error 等）
- 动态生成：根据用户参数填充模板并编译
- 决策依据：函数名 + 动作类型 → 选择模板
- 多实验并行：不同 CR 加载不同 eBPF 程序，相互独立
